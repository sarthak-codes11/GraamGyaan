...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const title = String(fields.title ?? "Untitled note");
    ...
    const notesDir = path.join(publicDir, "uploads", "notes");
    ensureDir(notesDir);

    const unitNumberRaw = fields.unitNumber ? Number(String(fields.unitNumber)) : undefined;
    const unitNumber = Number.isFinite(unitNumberRaw) ? (unitNumberRaw as number) : undefined;

    const savedItems = [] as any[];
    for (let i = 0; i < fileList.length; i++) {
      const f = fileList[i]!;
      const origName = f.originalFilename ?? `file-${i + 1}.pdf`;
      const stamped = `${Date.now()}-${i}-${safeFilename(origName)}`;
      const destPath = path.join(notesDir, stamped);
      await fs.promises.copyFile(f.filepath, destPath);
      await fs.promises.unlink(f.filepath).catch(() => {});
      const relativePath = `/uploads/notes/${stamped}`;
      const saved = addItem({
        id: `${Date.now()}-${i}`,
        type: "note",
